...
import { receitaFormSchema, type ReceitaFormData } from '@/src/lib/validations/forms'
import { FormField, FormSectionGroup, FormActions } from '@/components/ui/form-field'
import { Input } from '@/components/ui/input'

interface ReceitaFormProps {
  initialData?: ReceitaFormData
  onSubmit: (data: ReceitaFormData) => Promise<{ success?: boolean; error?: string }>
  onCancel?: () => void
}

export function ReceitaForm({ initialData, onSubmit, onCancel }: ReceitaFormProps) {
  const [isPending, startTransition] = useTransition()
  const [error, setError] = useState<string | null>(null)
  const [errors, setErrors] = useState<Record<string, string>>({})
  const [formData, setFormData] = useState<Partial<ReceitaFormData>>(initialData || {})

  const handleChange = (field: keyof ReceitaFormData, value: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        const validatedData = receitaFormSchema.parse(formData)
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-6 space-y-6">
      ...
      <FormSectionGroup title="Receita">
